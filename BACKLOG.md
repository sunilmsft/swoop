...
- [ ] 🟢 **Replace Twilio TFV console form with API-based resubmission flow** — every form session re-introduces the email-reset bug (now thrice-confirmed). The Trust Hub REST API (`/v1/CustomerProfiles`, `/v1/TollFreeVerifications`) lets you POST/PATCH the same fields without the broken UI. Build a small Node script that takes a JSON spec of the submission and runs it via API. Defensive: keep the spec under version control. _Jordan — "Three rounds on the same form-bug is the line."_
