...
- [ ] 🟡 **Replace Twilio TFV console form with API-based resubmission flow** — priority bumped 🟢→🟡 after Round 3 dash-strip discovery. The Trust Hub REST API (`/v1/CustomerProfiles`, `/v1/TollFreeVerifications`) lets you POST/PATCH the same fields without the broken UI sanitizer. Build a small Node script that takes a JSON spec of the submission and runs it via API; verify with a GET that the saved BRN includes the dash before submitting for review. Keep the spec under version control. _Jordan — "Three rounds on the same form-bug is the line."_ _Pursue this in parallel with Ignacio's manual-override response — whichever lands first wins._
- [ ] 🟡 **PARALLEL: begin A2P 10DLC brand vetting** — Round 3 dead-end on the demo line means TFV is now demonstrably blocked on a UI bug. Per the June 13 strategic decision, production was always going to 10DLC. Brand vetting (~$4) + single shared campaign (~$10/mo) on the BP-approved entity. If 10DLC clears, Swoop ships regardless of what happens with the 833 demo line.
